...
We observed seedling survival in a in a forest.  At each seedling, we measure an integrated metric of light levels.  We want to estimate a logistic regression model for these data to determine the relationship between light levels and seedlng survival.  The model will be of the form:
...
nllBinom<-function(parVec,sdlgs,lght){
    b0<-parVec[1]
    b1<-parVec[2]
  sdlgPred<-exp(b0+b1*lght) / (1+exp(b0+b1*lght)) #logistic regression
...
outBinom$par # beta0: -1.02  beta1: 3.086 looks good
outBinom$val # nllik 593.88
myAIC2<-2*2 + 2*outBinom$val # 1191.762

# plot regression fit to data
lightPred2<-light[order(light)]
plot(light,seedlingSurv)
ybin <-exp((outBinom$par[1]+outBinom$par[2]*lightPred2)) / (1+exp(outBinom$par[1]+outBinom$par[2]*lightPred2))
lines(lightPred2, ybin,col='red',lwd=2)
...
### 2. Compare your maximum likelihood fit to the R glm function.
```{r}
outBinomglm<-glm(seedlingSurv~light, family=binomial,data=seedlingSurvData)
summary(outBinomglm) #beta0: -1.022  beta1: 3.086
...
# default priors
outBinomStan <- brm(formula = seedlingSurv ~ light,
            data = seedlingSurvData, family = binomial())

summary(outBinomStan) # beta0: -1.03  beta1: 3.10
plot(outBinomStan)

# change priors from default
outBinomStan1 <- brm(formula = seedlingSurv ~ light,
            data = seedlingSurvData, family = binomial(),
            prior = c(set_prior("normal(0,100)", class = "b")),
            warmup = 1000, iter = 2000, chains = 4)

summary(outBinomStan1) # beta0: -1.03  beta1: 3.10
plot(outBinomStan1)


plot(marginal_effects(outBinomStan1), points = TRUE)
...
summary(outBinomrstanarm) # beta0: -1.0  beta1: 3.1
...
# changed mu[i] for logistic regression and model to binomial
```

```{r}
library(rstan)
myData2<-list(seedlingSurv=seedlingSurv,light=light,N=length(light))

#resStanBinom <- stan(model_code = 'BinomReg.stan', data = myData2,
#                chains = 3, iter = 3000, warmup = 500, thin = 10)

resStanBinom <- stan(model_code = modelString, data = myData2,
                chains = 3, iter = 3000, warmup = 500, thin = 10)

summary(resStanBinom,par=c('beta0','beta1'))  # beta0: -1.026  beta1:  3.097


library(coda)
post_fit2<-As.mcmc.list(resStanBinom) 
plot(post_fit2)
```
...
```{r}
# jags model string for binomial
# for loop part sets up distributions. binomial needs logit. after that are priors.
modelString<-"
model {
  for (i in 1:N){
...
```{r}
library(rjags)
jagsBinom <- jags.model('BinomReg.jags.txt',
...
update(jagsBinom, 1000)

jags.samples(jagsBinom,
             c('beta0', 'beta1'),
             10000)
...
library(coda)
codaSamples1<-coda.samples(jagsBinom, c('beta0','beta1'), 10000, 1)

plot(codaSamples1, trace = FALSE, density = TRUE)
summary(codaSamples1)  # beta0: -1.305  b1: 3.092
traceplot(codaSamples1)
