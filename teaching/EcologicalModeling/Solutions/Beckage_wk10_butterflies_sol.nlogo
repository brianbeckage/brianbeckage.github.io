...
to go
  ask turtles [move]
  update-outputs ; Exercise 3 Ch 9
  tick
  if ticks >= 1000 [
...
to update-outputs
  ...
  ; plot 2
  set-current-plot "mean elevation of butterflies"
  let yMin min [elevation] of patches
  let yMax max [elevation] of patches
  set-plot-y-range yMin yMax
  ;let meanElevation mean-butterfly-elevation
  ;plot meanElevation
  plot mean-butterfly-elevation

  ; plot 3 ; Exercise 4 Ch 9
  set-current-plot "elevation of butterflies"
  let xMin min [elevation] of patches
  let xMax max [elevation] of patches
...
to-report mean-butterfly-elevation
 report mean [elevation] of turtles
  ; show min [elevation] of patches
  ; show max [elevation] of patches
end
...
@#$#@#$#@
<experiments>
  <experiment name="q to maximize neighbors" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean-Neighbors</metric>
    <enumeratedValueSet variable="exportCorridorWidth?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="ButterFliesAtOrigin?">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="ImportElevationData?">
      <value value="true"/>
    </enumeratedValueSet>
